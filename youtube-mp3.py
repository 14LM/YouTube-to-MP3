...
enlace = eg.enterbox("Introduzca el enlace de Youtube que desea convertir", "Youtube a MP3")
...
if(re.search("^((?:https?:)?\/\/)?((?:www|m)\.)?((?:youtube\.com|youtu.be))(\/(?:[\w\-]+\?v=|embed\/|v\/)?)([\w\-]+)(\S+)?$", enlace)):
    try:
        ...
        print("La descarga no ha podido realizarse. Intente actualizar pytube (python3 -m pip install --upgrade pytube)")
else:
    print("El enlace no es válido, por favor intentelo nuevamente utilizando un enlace de Youtube")